...
let OrderUseCasesImpl = class OrderUseCasesImpl {
    ...
    async calculateDistancePriceBackground(orderId) {
        console.log(`[DistancePrice] Calculating distance & price for order ${orderId}`);
        const order = await this.orderRepo.getOrderCoordinates(orderId);
        if (!order)
            throw new Error(`[DistancePrice] Order ${orderId} not found`);
        const pickup = order.pickupAddress;
        const delivery = order.deliveryAddress;
        const branch = order.branch?.address;
        if (!delivery?.lat || !delivery?.long)
            throw new Error(`[DistancePrice] Invalid delivery coordinates for order ${orderId}`);
        const originCoords = pickup?.lat && pickup?.long
            ? { lat: parseFloat(pickup.lat), lon: parseFloat(pickup.long) }
            : branch
                ? { lat: parseFloat(branch.lat), lon: parseFloat(branch.long) }
                : null;
        if (!originCoords)
            throw new Error(`[DistancePrice] No valid origin coordinates for order ${orderId}`);
        const destinationCoords = {
            lat: parseFloat(delivery.lat),
            lon: parseFloat(delivery.long),
        };
        const distanceKm = await this.mapsService.calculateDistance(originCoords, destinationCoords);
        ...
        console.log(`[DistancePrice] Price calculation completed for order ${orderId}`);
    }
    async generateSegments(orderId) {
        console.log(`[Segments] Generating segments for order ${orderId}`);
        const order = await this.orderRepo.getOrderById(orderId);
        if (!order)
            throw new Error(`[Segments] Order ${orderId} not found`);
        const pickup = order.pickupAddress;
        const delivery = order.deliveryAddress;
        if (!delivery?.lat || !delivery?.long)
        ...
        if (order.shippingScope === 'TOWN') {
            const distanceKm = await this.mapsService.calculateDistance({ lat: parseFloat(pickup?.lat || '0'), lon: parseFloat(pickup?.long || '0') }, { lat: parseFloat(delivery.lat), lon: parseFloat(delivery.long) });
        ...
        if (order.shippingScope === 'REGIONAL' || order.shippingScope === 'INTERNATIONAL') {
            ...
            if (pickup) {
                const dist = await this.mapsService.calculateDistance({ lat: parseFloat(pickup.lat), lon: parseFloat(pickup.long) }, { lat: branchAddress.lat, lon: branchAddress.long });
                segmentsToCreate.push({
                    orderId,
                    originId: pickup.id,
            ...
            let dist = await this.mapsService.calculateDistance({ lat: branchAddress.lat, lon: branchAddress.long }, { lat: airportAddress.lat, lon: airportAddress.long });
            segmentsToCreate.push({
                orderId,
            ...
            totalDistance += dist;
            dist = await this.mapsService.calculateDistance({ lat: airportAddress.lat, lon: airportAddress.long }, { lat: branchAddress.lat, lon: branchAddress.long });
            segmentsToCreate.push({
                orderId,
            ...
            totalDistance += dist;
            dist = await this.mapsService.calculateDistance({ lat: branchAddress.lat, lon: branchAddress.long }, { lat: parseFloat(delivery.lat), lon: parseFloat(delivery.long) });
            segmentsToCreate.push({
                orderId,
                originId: branchAddress.id,
                destinationId: delivery.id,
    ...
    async resolveCustomerReceiver(data, userId) {
        let customer = (data.customerId &&
            (await this.orderRepo.findCustomer(data.customerId))) ||
            ((data.email || data.phone) &&
                (await this.orderRepo.findCustomerByEmailOrPhone(data.email, data.phone)));
