...
exports.DashboardReportRepository = void 0;
const common_1 = require("@nestjs/common");
const client_1 = require("@prisma/client");
const prisma_service_1 = require("../../../prisma/prisma.service");
let DashboardReportRepository = class DashboardReportRepository {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async getOverview() {
        const [totalOrders, pendingOrders, canceledOrders, deliveredOrders, failedOrders, totalRevenue, totalDrivers, totalVehicles, totalBranches,] = await Promise.all([
            ...
            this.prisma.order.count({
                where: { status: client_1.OrderStatus.CANCELED },
            }),
            this.prisma.order.count({
                where: { status: client_1.OrderStatus.DELIVERED },
            }),
            this.prisma.order.count({
                where: { status: client_1.OrderStatus.FAILED },
            }),
        ...
        const avgDeliveryTimeHours = deliveredOrdersData.length > 0
            ? deliveredOrdersData.reduce((sum, o) => {
                const durationMs = new Date(o.deliveryDate).getTime() -
    ...
    async getShipmentPerformance() {
        ...
        const rate = (count) => total > 0 ? Number(((count / total) * 100).toFixed(2)) : 0;
        ...
        const avgDeliveryTimeHours = deliveredOrders.length > 0
            ? deliveredOrders.reduce((sum, o) => {
                const diffMs = new Date(o.deliveryDate).getTime() -
                    new Date(o.pickupDate).getTime();
    ...
    async getBranchPerformance() {
        ...
        const revenueByBranch = payments.reduce((acc, p) => {
            ...
            if (branchId)
                acc[branchId] = (acc[branchId] || 0) + p.amount;
            return acc;
        }, {});
        const result = branches.map((branch) => {
            const stats = orderStats.filter((o) => o.branchId === branch.id);
            const completedOrders = stats.find((s) => s.status === 'DELIVERED')?._count.id || 0;
            const delayedOrders = stats.find((s) => s.status === 'EXCEPTION')?._count.id || 0;
            const revenue = revenueByBranch[branch.id] || 0;
            const activeOrderCount = activeOrders.filter((o) => o.branchId === branch.id).length;
            const totalOrders = completedOrders + delayedOrders + activeOrderCount;
            const efficiency = totalOrders === 0
                ? 0
                : Math.round((completedOrders / totalOrders) * 100);
            const trend = efficiency >= 85
                ? 'Improving'
                : efficiency >= 70
                    ? 'Stable'
                    : 'Declining';
    ...
    async getDriverPerformance(branchId) {
        ...
        for (const driver of drivers) {
            ...
            for (const order of orders) {
                ...
                if (order.pickupDriverId === driver.id) {
                    stat.pickupHandled++;
                    const pickupOnTime = order.actualPickupDate &&
                        order.pickupDate &&
                ...
                if (order.deliveryDriverId === driver.id) {
                    ...
                    if (order.actualPickupDate && order.actualDeliveryAt) {
                        const duration = (new Date(order.actualDeliveryAt).getTime() -
                            new Date(order.actualPickupDate).getTime()) /
                            60000;
                        stat.totalMinutes += duration;
                        const deliveryOnTime = order.actualDeliveryAt &&
        ...
        const report = drivers.map((driver) => {
            const stat = driverStats.get(driver.id);
            const avgTime = stat.totalMinutes && stat.totalDistance
            ...
            const onTimePercent = stat.handledOrders
                ...
                : 0;
            const status = stat.failedOrders > stat.completedOrders ? 'Underperforming' : 'Good';
            return {
                driverId: driver.id,
        ...
        return report.sort((a, b) => b.completedOrders - a.completedOrders ||
            b.onTimePercent - a.onTimePercent);
    }
    async getDriverSuccessRate(driverUserId) {
    ...
    async getBranchDashboardSummary() {
        ...
        const activeOrdersPercentChange = activeOrdersLastMonth
            ? Math.round(((activeOrdersThisMonth - activeOrdersLastMonth) /
    ...
    async getOrderCountBetween(start, end) {
        return this.prisma.order.count({
            where: { createdAt: { gte: start, lt: end } },
        });
    }
    async getDeliveredOrders(start, end) {
    ...
    async getOrderCounts(start, end) {
    ...
    async getNewCustomers(start, end) {
        return this.prisma.user.count({
            where: {
                customerType: 'INDIVIDUAL',
                isStaff: false,
    ...
    async getNewCorporateClients(start, end) {
        return this.prisma.user.count({
            where: {
                customerType: 'CORPORATE',
                isStaff: false,
    ...
    async getLogsBetween(start, end) {
...
exports.DashboardReportRepository = DashboardReportRepository;
exports.DashboardReportRepository = DashboardReportRepository = __decorate([
