...
export class DriverLocationService {
  ...
  async updateDriverLocation(data: {
    ...
    try {
      ...
      // Notify subscribed clients (always)
  ...
  for (const o of orders) {
    ...
    if (o.fulfillmentType === 'PICKUP') {
      if (pickup) coordsForDistance.push(pickup);
      if (o.shippingScope === 'TOWN' && delivery) coordsForDistance.push(delivery);
    } 
  ...
  // Map Redis results
  const nearbyMap = new Map<string, { distanceKm: number; lon: number; lat: number }>();
  const userIds: string[] = [];
  for (const d of rawDrivers) {
    const member = d[0];
    const distanceKm = parseFloat(d[1]);
  ...
  for (const drv of driverRecords) {
    ...
    for (const o of orders) {
      ...
      if (o.fulfillmentType === 'PICKUP') {
        ...
        else if (pickup) dist = this.calcDistanceKm(geo.lat, geo.lon, pickup.lat, pickup.lon);
        else if (delivery) dist = this.calcDistanceKm(geo.lat, geo.lon, delivery.lat, delivery.lon);
      } else if (o.fulfillmentType === 'DROPOFF') {
        if (delivery) dist = this.calcDistanceKm(geo.lat, geo.lon, delivery.lat, delivery.lon);
      }
